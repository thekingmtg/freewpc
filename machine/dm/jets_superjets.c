...
U8 						super_jet_goal;
__local__ U8 			superjets_modes_achieved; // number of times we entered the mode
...
CALLSET_ENTRY (jets_superjets, sw_jet) {
	if (super_jets_activated){
		...
		else /* goal not met yet */  					deff_start (DEFF_SUPERJETS_EFFECT);
	}
...
CALLSET_ENTRY (jets_superjets, sw_claw_super_jets) {
	...
	//TODO: music call doesn't work here
...
/****************************************************************************
 * DMD display effects - super jets
 ****************************************************************************/
void superjets_effect_deff(void) {
	dmd_alloc_low_clean ();
