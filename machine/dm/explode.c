...
//constants
const U8 EXPLODE_TIMER_DEFAULT = 43;
...
void explode_mode_init (void) {
	...
	task_sleep(TIME_2S);//wait for served ball so that explode made not trigger by it
...
CALLSET_BOOL_ENTRY (explode, ball_drain) { //thrown by device.c
	if	(flag_test(FLAG_IS_EXPLODE_MODE_RUNNING)
		&& explode_ballsave)	{
...
CALLSET_ENTRY (explode, end_ball)	 	{ if (timed_mode_running_p(&explode_mode) ) explode_mode_exit(); }
...
void explode_lights_task (void) {
	...
	for (i = 0; i < 5; i++) {
		...
		if (i % 2 == 0) sound_start (ST_SAMPLE, EXPLOSION, SL_1S, PRI_GAME_QUICK5);
	}
	task_sleep (TIME_133MS);
	sound_start (ST_SAMPLE, EXPLOSION, SL_1S, PRI_GAME_QUICK5);
	task_exit();
...
void explode_hit_effect_deff(void) {
	...
	switch (++explode_MessageCounter % 4) {
		default:
		case 0:
			for (fno = IMG_FORTRESS_C_START; fno <= IMG_FORTRESS_C_MID-2; fno += 2) {
				dmd_alloc_pair_clean ();// Clean both pages
				frame_draw(fno);
				dmd_show2 ();
				...
				font_render_string_center (&font_lithograph, DMD_MIDDLE_X, DMD_BIG_CY_Bot, "BOOM BABY");
				dmd_text_outline ();
				dmd_alloc_pair ();
				frame_draw(IMG_FORTRESS_C_MID);
				dmd_overlay_outline ();
				dmd_show2 ();
				task_sleep (TIME_200MS);
			break;
		case 1:
			for (fno = IMG_FORTRESS_C_MID; fno <= IMG_FORTRESS_C_END-2; fno += 2) {
				dmd_alloc_pair_clean ();// Clean both pages
				frame_draw(fno);
				dmd_show2 ();
				task_sleep (TIME_66MS);
		...
		case 2:
			for (fno = IMG_FORTRESS_C_MID; fno <= IMG_FORTRESS_C_END-2; fno += 2) {
				...
				dmd_clean_page_high ();//
				font_render_string_center (&font_var5, DMD_MIDDLE_X, DMD_SMALL_CY_1, "EXPLODE");
				font_render_string_center (&font_lithograph, DMD_MIDDLE_X, DMD_BIG_CY_Bot, "BOOM BOOM");
				dmd_text_outline ();
				dmd_alloc_pair ();
				frame_draw(IMG_FORTRESS_C_END);
				dmd_overlay_outline ();
...
//			font_render_string_center (&font_var5, DMD_MIDDLE_X, DMD_SMALL_CY_1, sprintf_buffer);
			...
			dmd_show_low ();
			task_sleep (TIME_500MS);//score goes down by second so update every xxms is plenty soon enough
	}//END OF ENDLESS LOOP
}//end of mode_effect_deff
