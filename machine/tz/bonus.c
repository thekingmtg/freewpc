...
void one_ball_score_task (void)
{
	...
	if (num_players > 1 && player_up == num_players && ball_up != 1 && extra_balls == 0 && feature_config.adv_bonus_info == YES)
	{
		...
		dmd_alloc_low_clean ();
		font_render_string_center (&font_var5, 64, 4, "HIGHEST 1 BALL SCORE");
...
//	one_ball_score_task ();
	...
	if (system_config.lowest_goes_next == NO)
	{
...

		{
			...
			{
				font_render_string_center (&font_var5, 64, 29, "THAT WAS CLOSE");
			}
			else if (score_compare (temp_score, score_table[SC_100M]) == 1)
			{
				font_render_string_center (&font_var5, 64, 29, "A LITTLE LOPSIDED");
			}
			else
			{
				font_render_string_center (&font_var5, 64, 29, "CONGRATULATIONS");
			}	
			dmd_show_low ();
			task_sleep_sec (5);
		}
...
{
	dmd_alloc_low_clean ();
	font_render_string_center (&font_var5, 64, 19, "POINTS NEEDED");
	if (find_player_ranked(1) + 1 == player_up)
	{
		font_render_string_center (&font_quadrit, 64, 8, "NO");
		if (check_if_last_ball_of_multiplayer_game ())
		{
			font_render_string_center (&font_var5, 64, 26, "SHOWBOAT A LITTLE");
		}
		else
		{
			font_render_string_center (&font_var5, 64, 26, "INCREASE YOUR LEAD");
		}
	}
