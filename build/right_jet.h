...
extern __fastram__ U8 right_jet_timer;

void right_jet_enable (void);
void right_jet_disable (void);

extern inline U8 right_jet_enabled_p (void)
{
	return jet_enables & INST_MASK;
}


extern inline U8 right_jet_running_p (void)
{
	return jet_running & INST_MASK;
}


extern inline void right_jet_service (void)
{
	...
	if (unlikely (right_jet_timer))
	{
		...
		if (right_jet_timer == 0)
		{
			...
			else
			{
				...
				if (rt_switch_poll (SW_RIGHT_JET))
				{
					right_jet_timer = 16;
				}
			}
	...
	else if (right_jet_enabled_p () && rt_switch_poll (SW_RIGHT_JET))
	{
		/* When the solenoid is not running, poll its switch
		and see if it should be started. */
		sol_enable (SOL_RIGHT_JET);
		jet_running |= INST_MASK;
		right_jet_timer = 3;
	}
}
